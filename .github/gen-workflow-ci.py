...
def main():
    ...
    def publish_docker_images(needs: List[str], images: List[str]) -> str:
        ...
        return (f'  start-runner:\n'
                f'    name: Start self-hosted EC2 runner\n'
                f'    needs: [{", ".join(needs)}]\n'
                f'    if: >\n'
                f'      (\n'
                ...
                f'    runs-on: ubuntu-latest\n'
                f'    outputs:\n' +
                '\n'.join([f'      label-{image}: ${{{{ steps.start-ec2-runner-{image}.outputs.label }}}}\n'
                           f'      ec2-instance-id-{image}: ${{{{ steps.start-ec2-runner-{image}.outputs.ec2-instance-id }}}}\n'
                           for image in images]) +
                f'\n'
                f'    steps:\n'
                f'      - name: Configure AWS credentials\n'
                ...
                f'\n' +
                '\n'.join([f'      - name: Start EC2 runner ({image})\n'
                           f'        id: start-ec2-runner-{image}\n'
                ...
                f'\n'
                f'  docker:\n'
                f'    name: docker ${{{{ matrix.docker-image }}}}\n'
                ...
                f'\n'
                f'  stop-runner:\n'
                f'    name: Stop self-hosted EC2 runner\n'
                ...
                f'          aws-region: ${{{{ secrets.AWS_REGION }}}}\n'
                f'\n' +
                '\n'.join([f'      - name: Stop EC2 runner ({image})\n'
                           f'        uses: machulav/ec2-github-runner@v2.2.0\n'
                           f'        with:\n'
                           f'          mode: stop\n'
                           f'          github-token: ${{{{ secrets.GH_PERSONAL_ACCESS_TOKEN }}}}\n'
