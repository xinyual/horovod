...
def main():
    ...
    def publish_docker_images(needs: List[str], images: List[str]) -> str:
        if 'init-workflow' not in needs:
            needs.insert(0, 'init-workflow')
        return (f'  start-runner:\n'
                f'    name: Start self-hosted EC2 runner (${{{{ matrix.docker-image }}}})\n'
                f'    needs: [{", ".join(needs)}]\n'
                f'    if: >\n'
                f'      (\n'
                f'        github.event_name == \'push\' && github.ref == \'refs/heads/master\' ||\n'
                f'        github.event_name == \'push\' && startsWith(github.ref, \'refs/tags/v\') && contains(github.ref, \'.\') ||\n'
                f'        github.event_name == \'pull_request\' && github.event.pull_request.base.ref == \'master\' && needs.init-workflow.outputs.run_builds_and_tests != \'false\'\n'
                f'      ) && ! github.event.repository.fork\n'
                f'    runs-on: ubuntu-latest\n'
                f'\n'
                f'    strategy:\n'
                f'      fail-fast: false\n'
                f'      matrix:\n'
                f'        docker-image: [horovod, horovod-cpu, horovod-ray]\n'
                f'\n'
                f'    outputs:\n' +
                '\n'.join([f'      label-{image}: ${{{{ steps.ec2-meta.outputs.label-{image} }}}}\n'
                           f'      ec2-instance-id-{image}: ${{{{ steps.ec2-meta.outputs.ec2-instance-id-{image} }}}}\n'
                           for image in images]) +
                f'\n'
                f'    steps:\n'
                f'      - name: Configure AWS credentials\n'
                ...
                f'          aws-access-key-id: ${{{{ secrets.AWS_ACCESS_KEY_ID }}}}\n'
                f'          aws-secret-access-key: ${{{{ secrets.AWS_SECRET_ACCESS_KEY }}}}\n'
                f'          aws-region: ${{{{ secrets.AWS_REGION }}}}\n'
                f'\n'
                f'      - name: Start EC2 runner (${{{{ matrix.docker-image }}}})\n'
                f'        id: start-ec2-runner\n'
                f'        uses: machulav/ec2-github-runner@v2.2.0\n'
                f'        with:\n'
                f'          mode: start\n'
                f'          github-token: ${{{{ secrets.GH_PERSONAL_ACCESS_TOKEN }}}}\n'
                f'          ec2-image-id: ami-0759580dedc953d1f\n'
                f'          ec2-instance-type: r5.large\n'
                f'          subnet-id: subnet-0983be43\n'
                f'          security-group-id: sg-4cba0d08\n'
                f'\n'
                f'      - name: Get EC2 instance meta\n'
                f'        id: ec2-meta\n'
                f'        run: |\n'
                f'          echo "::set-output name=label-${{{{ matrix.docker-image }}}}::${{{{ steps.start-ec2-runner.outputs.label }}}}"\n'
                f'          echo "::set-output name=ec2-instance-id-${{{{ matrix.docker-image }}}}::${{{{ steps.start-ec2-runner.outputs.ec2-instance-id }}}}"\n'
                f'\n'
                f'  docker:\n'
                f'    name: Build docker image ${{{{ matrix.docker-image }}}} (push=${{{{ github.event_name != \'pull_request\' }}}})\n'
                f'    if: needs.start-runner.result != \'skipped\'\n'
                f'    needs: start-runner # required to start the main job when the runner is ready\n'
                f'    runs-on: ${{{{ matrix.label }}}} # run the job on the newly created runners\n'
                f'\n'
                f'    strategy:\n'
                f'      fail-fast: false\n'
                f'      matrix:\n'
                f'        include:\n' +
                '\n'.join([f'          - docker-image: {image}\n'
                           f'            label: ${{{{ needs.start-runner.outputs.label-{image} }}}}\n'
                           for image in images]) +
                f'\n'
                f'    steps:\n'
                f'      - name: Checkout\n'
                ...
                f'          push: ${{{{ github.event_name != \'pull_request\' }}}}\n'
                f'          tags: ${{{{ steps.meta.outputs.tags }}}}\n'
                f'          labels: ${{{{ steps.meta.outputs.labels }}}}\n'
                f'\n'
                f'  stop-runner:\n'
                f'    name: Stop self-hosted EC2 runner (${{{{ matrix.docker-image }}}})\n'
                f'    # always() is required to stop the runner even if previous jobs failed\n'
                f'    if: always() && needs.start-runner.result != \'skipped\'\n'
                f'    needs:\n'
                f'      - start-runner # required to get output from the start-runner job\n'
                f'      - docker # required to wait when the main job is done\n'
                f'    runs-on: ubuntu-latest\n'
                f'\n'
                f'    strategy:\n'
                f'      fail-fast: false\n'
                ...
                '\n'.join([f'          - docker-image: {image}\n'
                           f'            label: ${{{{ needs.start-runner.outputs.label-{image} }}}}\n'
                           f'            ec2-instance-id: ${{{{ needs.start-runner.outputs.ec2-instance-id-{image} }}}}\n'
                           for image in images]) +
                f'\n'
                f'    steps:\n'
                f'      - name: Configure AWS credentials\n'
                ...
                f'          aws-region: ${{{{ secrets.AWS_REGION }}}}\n'
                f'\n' +
                f'      - name: Stop EC2 runner (${{{{ matrix.docker-image }}}})\n'
                f'        uses: machulav/ec2-github-runner@v2.2.0\n'
                f'        with:\n'
                f'          mode: stop\n'
                f'          github-token: ${{{{ secrets.GH_PERSONAL_ACCESS_TOKEN }}}}\n'
                f'          label: ${{{{ matrix.label }}}}\n'
                f'          ec2-instance-id: ${{{{ matrix.ec2-instance-id }}}}\n')
